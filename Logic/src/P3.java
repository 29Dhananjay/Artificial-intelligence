import java.util.*;
/**
 * P3 class: implimentation of the intermediate agent solver using DNF logic which uses the logic NG library.
...
public class P3 {
	...
	public static void solver(char[][] board,char[][] current_map,Boolean verbose,HashMap<Integer, Character> map,int board_size,int mine_size, int blocked_size,Boolean game_won){
		...
		for (int clue_state: clue_states){
			...
			// generating KBU and expanding KBU
			String sentence = Utility.sentenceConstructorDNF(clue,combo, arr,unprobed_neighbours);
			KBU += "(" + sentence + ")" + AND;
		...
		for (int unprobed: unprobed_states){
			...
			if (Utility.proveMineorFreeDNF(KBU, unprobed) == false){
